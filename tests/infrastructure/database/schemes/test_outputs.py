...
@pytest.mark.parametrize(
    ...
    [
        ...
        (OutputTypeMeasure.NEW_SHARED_USE_WALKING_WHEELING_AND_CYCLING_FACILITIES_MILES, 5),
        (OutputTypeMeasure.NEW_SHARED_USE_WALKING_AND_CYCLING_FACILITIES_MILES, 6),
...
class TestOutputTypeMeasureMapper:
    def test_to_id(self, output_type_measure: OutputTypeMeasure, id_: int) -> None:
        assert OutputTypeMeasureMapper().to_id(output_type_measure) == id_

    def test_to_domain(self, output_type_measure: OutputTypeMeasure, id_: int) -> None:
        assert OutputTypeMeasureMapper().to_domain(id_) == output_type_measure
